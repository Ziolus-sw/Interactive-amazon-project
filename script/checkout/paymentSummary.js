...
export function renderPaymentSummary() {
  ...
  function finalRendring() {
    ...
    document.querySelector(
      ".js-payment-summary-row-1 :nth-child(1)",
    ).textContent = `Items (${countCartQuantity()}):`;
    document.querySelector(".js-total-price").textContent =
      `${(price / 100).toFixed(2)}`;
    document.querySelector(".js-shipping-cost").textContent =
      `${(shippingCost / 100).toFixed(2)}`;
    document.querySelector(".js-total-value-before-tax").textContent =
      `${((price + shippingCost) / 100).toFixed(2)}`;
    document.querySelector(".js-tax").textContent = `${(tax / 100).toFixed(2)}`;
    document.querySelector(".js-total-value-after-tax").textContent =
      `${((price + shippingCost + tax) / 100).toFixed(2)}`;
  }
